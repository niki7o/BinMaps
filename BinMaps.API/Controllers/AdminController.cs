...
public sealed class AdminController : ControllerBase
{
    ...
    #region Diagnostics — external API health

    /// <summary>
    /// Pings each external API the system depends on and reports its status.
    /// Used by the admin dashboard's "API Health" panel so we can confirm at
    /// a glance whether weather, routing, and the AI service are reachable.
    /// Each check is bounded by a short timeout so a slow upstream cannot
    /// hang the diagnostics call. Always returns 200 with per-service detail.
    /// </summary>
    [HttpGet("diagnostics/apis")]
    public async Task<IActionResult> CheckExternalApis(
        [FromServices] IExternalWeatherService weather,
        [FromServices] IExternalRoutingService routing,
        [FromServices] IConfiguration config,
        [FromServices] IHttpClientFactory httpFactory)
    {
        // Geographic point we use for the smoke tests — Sofia centre, matches
        // the seeded data so we exercise the same code path as production.
        const double lat = 42.6977;
        const double lng = 23.3219;

        var weatherCheck = await TimeOperationAsync(async () =>
        {
            var temp = await weather.GetAmbientTemperatureAsync(lat, lng);
            return temp is null
                ? (false, "no data returned")
                : (true, $"temperature: {temp.Value:F1}°C");
        });

        var routingCheck = await TimeOperationAsync(async () =>
        {
            var origins = new[] { new GeoCoordinate(lat, lng) };
            var dests = new[] { new GeoCoordinate(lat + 0.01, lng + 0.01) };
            var matrix = await routing.GetMatrixAsync(origins, dests);

            var rawKey = config["ExternalAPIs:TomTom:ApiKey"];
            var keyConfigured = !string.IsNullOrWhiteSpace(rawKey)
                && !rawKey.Equals("YOUR_TOMTOM_API_KEY", StringComparison.OrdinalIgnoreCase);

            if (matrix is null)
                return keyConfigured
                    ? (false, "matrix call failed — using Haversine fallback")
                    : (false, "no API key — Haversine fallback active");

            return (true, "matrix call succeeded");
        });

        var aiEndpoint = config["AISettings:Endpoint"] ?? "http://localhost:8000/analyze";
        var aiHealthUrl = aiEndpoint.Replace("/analyze", "/health");
        var aiCheck = await TimeOperationAsync(async () =>
        {
            using var http = httpFactory.CreateClient();
            http.Timeout = TimeSpan.FromSeconds(4);
            try
            {
                var resp = await http.GetAsync(aiHealthUrl);
                if (!resp.IsSuccessStatusCode)
                    return (false, $"AI /health returned {(int)resp.StatusCode}");
                return (true, $"AI /health OK ({aiHealthUrl})");
            }
            catch (TaskCanceledException)
            {
                return (false, "AI /health timed out");
            }
            catch (Exception ex)
            {
                return (false, $"AI /health unreachable: {ex.GetType().Name}");
            }
        });

        return Ok(new
        {
            weather  = new { name = "Open-Meteo (temperature)", weatherCheck.Ok, weatherCheck.Detail, weatherCheck.ElapsedMs },
            routing  = new { name = "TomTom (matrix routing)",  routingCheck.Ok, routingCheck.Detail, routingCheck.ElapsedMs },
            ai       = new { name = "BinMaps AI (FastAPI)",     aiCheck.Ok,      aiCheck.Detail,      aiCheck.ElapsedMs },
            checkedAt = DateTime.UtcNow,
        });
    }

    /// <summary>Helper that times a check and translates exceptions into a
    /// uniform (Ok, Detail, ElapsedMs) tuple so the controller stays tidy.</summary>
    private static async Task<(bool Ok, string Detail, long ElapsedMs)> TimeOperationAsync(
        Func<Task<(bool, string)>> op)
    {
        var sw = Stopwatch.StartNew();
        try
        {
            var (ok, detail) = await op();
            sw.Stop();
            return (ok, detail, sw.ElapsedMilliseconds);
        }
        catch (Exception ex)
        {
            sw.Stop();
            return (false, $"{ex.GetType().Name}: {ex.Message}", sw.ElapsedMilliseconds);
        }
    }
